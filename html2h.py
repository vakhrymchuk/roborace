...
if n_pages > 0:
    ...
    print("", file=sys.stderr)
    print('#include "web.h"', file=sys.stderr)
    print("", file=sys.stderr)
    for n ,ident in enumerate(idents):
        print("const Page page_%s = {%s_name, %s, %s_len - 1};" % (ident, ident, ident, ident), file=sys.stderr)

    print("", file=sys.stderr)

    print("const Page* const pages[] = {", file=sys.stderr)
    # for n in xrange(1, n_pages + 1):
    #     print("\t&page%02d," % (n - 1), file=sys.stderr)
    for n, ident in enumerate(idents):
        print("    &page_%s," % (ident), file=sys.stderr)

    # print("\tNULL", file=sys.stderr)
    print("};", file=sys.stderr)
    print("", file=sys.stderr)
    print("const byte pagesCount = (byte) (sizeof(pages) / sizeof(Page *));", file=sys.stderr)
